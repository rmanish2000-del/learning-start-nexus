...
Columns O–U are the only editable cells (green):

| Column | Rule |
| --- | --- |
| Expert decision | Drop-down: `APPROVE`, `REVISE`, `REJECT` |
| Corrected question | Required when the decision is `REVISE` and the stem or options are at fault — give the exact replacement wording |
| Corrected answer | Required when the decision is `REVISE` and the keyed answer is wrong — give the correct answer |
...
- **REVISE** — sound intent, defective execution; the corrected question and/or corrected answer column must contain the fix.
