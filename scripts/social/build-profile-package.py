...
Run: python3 scripts/social/build-profile-package.py
"""
...
def font(size: int, *, display: bool = True, weight: int = 700) -> ImageFont.FreeTypeFont:
    path = DISPLAY_FONT if display else BODY_FONT
...
def record(name, kind, platform, w, h, fmt, purpose, *, safe=None, canvas: Canvas | None = None,
    ...
    if canvas is not None and safe:
        ...
        VALIDATION.append(dict(file=entry["file"], text_boxes=len(canvas.text_boxes),
                               outside_safe_area=len(bad), ok=not bad))
...
def build_banners():
    ...
    c = brand_banner(1128, 191, (400, 20, 1104, 171), headline="EduOS", sub=SCOPE,
                     mark_size=96, mark_xy=(1004, 48), text_x=420, text_top=44, hl_size=52, sub_size=22, max_w=560)
    ...
    c = brand_banner(820, 312, (130, 30, 690, 282), headline="EduOS", sub=SCOPE,
                     mark_size=120, mark_xy=(150, 96), text_x=300, text_top=84, hl_size=64, sub_size=24, max_w=380)
    ...
    c = brand_banner(640, 360, (40, 40, 600, 320), headline="EduOS", sub=SCOPE,
                     mark_size=120, mark_xy=(60, 120), text_x=210, text_top=110, hl_size=60, sub_size=22, max_w=380)
...
def build_watermarks():
    for name, colour, purpose in [
        ...
        d = ImageDraw.Draw(wm)
        f = font(96, display=False, weight=600)
...
def pinned_card(w, h, name, platform, *, pad, hl, sub, max_w):
    ...
    c.text((pad, h - pad - int(sub * 1.1)), f"{PROOF['cta']}  ·  {SITE}", font(sub, display=False, weight=600), ORANGE)
    save(c.finish(), name)
...
def build_pinned():
    pinned_card(1200, 628, "pinned-linkedin-1200x628.png", "LinkedIn", pad=72, hl=54, sub=28, max_w=1000)
    pinned_card(1200, 630, "pinned-facebook-1200x630.png", "Facebook", pad=72, hl=54, sub=28, max_w=1000)
    pinned_card(1080, 1080, "pinned-instagram-1080x1080.png", "Instagram", pad=88, hl=70, sub=36, max_w=904)
    pinned_card(1600, 900, "pinned-x-1600x900.png", "X", pad=96, hl=72, sub=36, max_w=1300)
...
def sheet(title: str, tiles: list[tuple[str, Image.Image]], cols: int, tile_w: int, name: str):
    ...
    d = ImageDraw.Draw(s)
    d.text((pad, pad), title, font=font(40), fill=NEUTRAL_900)
    for i, (label, t) in enumerate(tiles):
        ...
        d.text((x, y + t.height + 8), label, font=font(24, display=False, weight=500), fill=NEUTRAL_500)
...
def write_docs(previews: list[str]):
    ...
    manifest = dict(
        ...
        version="1.0.0",
        source_kit=KIT["name"],
        generated_by="scripts/social/build-profile-package.py",
        fonts=FONT_REPORT,
