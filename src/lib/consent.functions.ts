...
import { guardianConsentSchema, revokeConsentSchema } from "@/lib/schemas";
...
export const recordGuardianConsent = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    // Staff who manage the learner, or the linked parent themselves. RLS
    // (consents_insert / consents_parent_insert) restricts the actual write.
...
// Withdraw consent: appends a revocation row so the history stays intact.
// The latest row wins, so a revocation immediately re-locks the AI Tutor.
export const revokeGuardianConsent = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input: unknown) => revokeConsentSchema.parse(input))
  .handler(async ({ data, context }) => {
    await requireAnyRole(context.supabase, context.userId, ["admin", "educator", "parent"]);
    ...
    if (!status.hasConsent || !status.latest) {
      throw new Error("There is no active consent to withdraw.");
    }
    ...
    // Carry the guardian identity from the active consent so the history
    ...
    const { error } = await context.supabase.from("guardian_consents").insert({
      ...
      consent_version: status.latest.consentVersion,
      recorded_by: context.userId,
      action: "revoked",
    });
    if (error) throw error;
